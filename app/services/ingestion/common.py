...
_VALID_TICKER_RE = re.compile(r"^[A-Z0-9_\-\.]{2,20}$")
_SKIP_TICKER_PREFIXES = (
    "COPYRIGHT", "TODOS", "DIREITOS", "RESERVADOS", "BODIVA.",
    "FONTE", "TOTAL", "SUBTOTAL", "LEGENDA", "NOTA", "OBS",
)


def normalize_ticker(value: Any) -> str:
    """Return uppercased ticker or empty string if value looks like legal/footer text."""
    raw = str(value or "").strip().upper()
    if not raw:
        return ""
    # Reject if it contains ©, ®, or common legal characters
    if any(c in raw for c in ("©", "®", "™", "§")):
        return ""
    # Reject if too long (no BODIVA ticker exceeds 20 chars)
    if len(raw) > 20:
        return ""
    # Reject if it starts with known non-ticker words
    if any(raw.startswith(p) for p in _SKIP_TICKER_PREFIXES):
        return ""
    # Reject if it contains multiple words (legal sentences have spaces)
    if raw.count(" ") >= 3:
        return ""
    # Accept only if it matches ticker pattern
    if not _VALID_TICKER_RE.match(raw.replace(" ", "_")):
        return ""
    return raw
